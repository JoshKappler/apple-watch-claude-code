...
Three pieces talking over one small WebSocket protocol:
...
└─────────────────┘                           └──────────────────────────┘
   ...
   shows diffs, confirms                         Cloudflare Tunnel
                                                 (works over cellular)
```

- **Backend** (`backend/`): a Node/TypeScript service wrapping the **Claude Agent SDK** — the same engine as the Claude Code CLI, so the watch gets the *same* tools and autonomy (Bash, Edit, Write, Read, Grep, ...). Exposes a WebSocket that streams assistant text, tool calls, and permission requests, and accepts voice-transcribed messages, approve/decline decisions, mode changes, and cancels. Supports `bypassPermissions` ("dangerously skip permissions").
...
- **Infra** (`infra/`): Cloudflare Tunnel config + a `launchd` agent so the backend + public URL come up on login and survive crashes.
...
- **Backend runs where your repos are (your Mac), exposed via Cloudflare Tunnel:** this keeps your real, possibly-uncommitted projects reachable, and the tunnel gives a stable public `https://` URL the watch hits over cellular. A cloud mode (clone from GitHub on a VM) is also supported for when your Mac is off — same backend, different working dir.
...
- An **Anthropic API key** (or Claude subscription auth) in `backend/.env`.
- **Xcode + an Apple Developer account** to sign and install the watch app on your Ultra. The Swift source is complete and ready to open; signing/installing is interactive and needs your account.
- Running the one-time **Cloudflare Tunnel** login (`cloudflared login`) to mint your public URL.
