...
3. Point the watch at your backend. Edit **`watch/Sources/Shared/Secrets.swift`**
...
- The repo is **public**. Secrets live in gitignored files only —
  `backend/.env` and `watch/Sources/Shared/Secrets.swift`. Never paste the token into
  committed source.
...
| `watch/` | Xcode project for BOTH native apps — `Pinch` (watchOS) + `PinchPhone` (iOS), generated by XcodeGen. `Sources/` splits into `Shared/` (the brain, both apps), `Watch/`, `Phone/`. The remotes. |
